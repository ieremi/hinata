...
(() => {
  ...
  // src/hard-range.ts
  var HardRange = class _HardRange extends Range {
    static readFrom(hashParams) {
      const min = hashParams.has("min") ? Helper.parseNumber(hashParams.get("min")) : null;
      const max = hashParams.has("max") ? Helper.parseNumber(hashParams.get("max")) : null;
      return new _HardRange(min, max);
    }
    get min() {
      return this.lo;
    }
  ...
  // src/soft-range.ts
  var SoftRange = class _SoftRange extends Range {
    static readFrom(hashParams, hardRange, initialA) {
      const a = hashParams.has("a") ? Helper.parseNumber(hashParams.get("a")) : hardRange.min ?? initialA;
      const b = hashParams.has("b") ? Helper.parseNumber(hashParams.get("b")) : hardRange.max;
      return new _SoftRange(a, b);
    }
    get a() {
      return this.lo;
    }
  ...
  var LoopPlayer = class {
    ...
    copy() {
      navigator.clipboard.writeText(location.href).catch(() => console.log("[AB LOOP] failed to copy URL to clipboard"));
    }
    ...
    tick() {
      ...
      if (this.softRange.a !== null && this.softRange.b !== null && video.currentTime >= this.softRange.b) {
